...
k = [0 : (N-1)/2]; subplot 211; stem(k, H(1:(N-1)/2+1)); grid;  axis([0 (N-1)/2+0.5 0 1]); xlabel('k'); title('Magnitud deseada [-]') 
% Grafica la respuesta de frecuencia obtenida 
subplot 212; plot(w/pi, HdB); grid;  axis([0 1 -50 5]); xlabel('w/pi'); title('Magnitud obtenida [dB]') zoom