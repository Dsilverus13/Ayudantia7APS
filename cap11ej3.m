...
m1 = 1e-6;  % límite inferior del gráfico de magnitud 
 ...
 f = [f1 : (f2-f1)/1023 : f2]; H = freqz(b, 1, f, Fs); 
 % Grafica 
 h(n) n = [1 : N]; subplot 211; stem(n, b); grid; xlabel('n'); title('h(n)') 
 % Grafica 
 Mag[H(f)] subplot 212; semilogy(f, abs(H)); grid axis([f1 f2 m1 1.1]); xlabel('f[Hz]'); title('Mag(H)') 